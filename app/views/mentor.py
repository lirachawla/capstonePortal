...
def change():
    try:
        ...
        if request.method == "POST":
            ...
            cur.execute("UPDATE Mentors SET password = %s WHERE email = %s",(phash, session["user_id"],))
        ...
        if (request.form.get("action") == 'sub_marks'):
            ...
            for student in students:
                ...
                if eval1 is not None:
                    cur.execute("update Students set evaluation1_marks= %s where roll_no=%s",(eval1,student[1],))
                if eval2 is not None:
                    cur.execute("update Students set evaluation2_marks= %s where roll_no=%s",(eval2,student[1],))
                if eval3 is not None:
                    cur.execute("update Students set evaluation3_marks= %s where roll_no=%s",(eval3,student[1],))
                if eval4 is not None:
                    cur.execute("update Students set evaluation4_marks= %s where roll_no=%s",(eval4,student[1],))
                if eval5 is not None:
                    cur.execute("update Students set evaluation5_marks= %s where roll_no=%s",(eval5,student[1],))
                if eval6 is not None:
                    cur.execute("update Students set evaluation6_marks= %s where roll_no=%s",(eval6,student[1],))
            mysql.connection.commit()
    ...
    try:
        ...
        if request.method== 'GET':
            ...
            if request.form.get("add", False):
                cur.execute('insert into Teams (leader_roll_no,title,student2_roll_no,student3_roll_no,student4_roll_no,email,phone) values(%s,%s,%s,%s,%s,%s,%s)',(
                    request_grp[0][1],
                    request_grp[0][2],
                    request_grp[0][3],
                    request_grp[0][4],
                ...
                grp_id = grp_id[0]
                cur.execute('insert into Group_Mentors (group_id,mentor1_id) values(%s,%s)',(
                    grp_id,
                    request_grp[0][12],
                    ))
                cur.execute('insert into Students (group_id,roll_no,name) values(%s,%s,%s)',(
                    grp_id,
                    request_grp[0][1],
                    request_grp[0][1+6],
                    ))
                for j in range(1,4):
                    cur.execute('insert into Panel_Marks (roll_no,panel_no) values(%s,%s)',(
                        request_grp[0][1],
                        j,
                        ))
                for i in range(3, 5):
                ...
                cur.execute("DELETE FROM Requests WHERE leader_roll_no = %s",(lead_id,))
                ## Composing Mail
                title="Capstone Project Group Request"
                sender="no-reply@thapar.edu"
                recipients = [request_grp[0][6]]
                message_html = "<b>Your Capstone Project Group request was successfully accepted.</b>"
                    ...
                    if len(student)>0:
                        for i in eval_details:
                            ...
                            if query_db("select parameter%s_marks from Panel_Marks where panel_no=%s and roll_no=%s",(i[0], eval_id, student[0], )):
                                if marks=='NULL':
                                    cur.execute("UPDATE Panel_Marks set parameter%s_marks=NULL where panel_no=%s and roll_no=%s",(i[0], eval_id, student[0], ))
                                else:
                                    cur.execute("UPDATE Panel_Marks set parameter%s_marks=%s where panel_no=%s and roll_no=%s",(i[0], marks,  eval_id, student[0], ))
                            else:
                                if marks=='NULL':
                                    cur.execute("Insert into Panel_Marks(parameter%s_marks, panel_no, roll_no) values(NULL, %s, %s)",(i[0], eval_id, student[0], ))
                                else:
                                    cur.execute("Insert into Panel_Marks(parameter%s_marks, panel_no, roll_no) values(%s, %s, %s)",(i[0], marks, eval_id, student[0], ))
                print(eval_id, panel_id, group_id)
                if filled==True:
                    cur.execute("UPDATE Panel_Group set filled=1 where panel_no=%s and panel_id=%s and group_id=%s",(eval_id, panel_id, group_id, ))
                else:
                    cur.execute("UPDATE Panel_Group set filled=0 where panel_no=%s and panel_id=%s and group_id=%s",(eval_id, panel_id, group_id, ))
                mysql.connection.commit()
        flash("Marks Updated Successfully!", 'success')
            ...
            if sha.verify(password, phash[0][0]):
                ...
                if mentor:
                    if query_db("select mentor2_id from Group_Mentors where group_id=%s",(group_id, ))[0][0] is not None:
                        ...
                        cur.execute("update Group_Mentors set mentor2_id=%s where group_id=%s",(mentor[0], group_id, ))
                        mysql.connection.commit()
...
def edit_group():
    try:
        ...
        if request.method=="POST":
            ...
            cur.execute("update Teams set title=%s where group_id=%s",(title_des,group_id))
            mysql.connection.commit()
