...
def clear_all():
    try:
        ...
        cur.execute("delete from Group_Mentors where 1=1")
        cur.execute("delete from Requests where 1=1")
        cur.execute("delete from Panel_group where 1=1")
        cur.execute("delete from Panelists where 1=1")
        cur.execute("delete from Panel_Marks where 1=1")
        cur.execute("delete from Panel_parameter where 1=1")
...
def full_marks():
    try:
        ...
        if request.method=='GET':
            ...
            if int(eval_id)<7:
                cur.execute("UPDATE Evaluations SET max_marks = %s WHERE evaluation_no = %s",(full_marks, eval_id,))
            else:
                eval_id-=6
                cur.execute("UPDATE panel_dates SET maxmarks = %s WHERE eval_id = %s",(full_marks, eval_id,))
            mysql.connection.commit()
        ...
        if request.method=='GET':
            ...
            if sha.verify(password, phash[0][0]):
                ...
                if is_admin:
                    ...
                    cur.execute("INSERT INTO Heads(mentor_id) VALUES(%s)",(mentorid[0][0], ))
                    mysql.connection.commit()
        ...
        if request.method=='GET':
            ...
            if not eval_id or not start_date or not end_date:
                ...
                if query_db("SELECT * from Evaluations where evaluation_no=%s",(eval_id,)):
                    cur.execute("UPDATE Evaluations SET start_date=%s, end_date=%s where evaluation_no=%s", (start_date, end_date, eval_id, ))
                else:
                    cur.execute("INSERT INTO Evaluations(start_date, end_date, evaluation_no, max_marks) values(%s,%s,%s, 5)", (start_date, end_date, eval_id, ))
            mysql.connection.commit()
        ...
        if request.method=="GET":
            ...
            if panels is not None:
                ...
                for i in range(1, panels_no+1):
                    for j in panels[i-1]:
                        for z in range(1, 4):
                            cur.execute("insert into Panel_Group values(%s,%s, %s)",(z,i, j[0], ))
                panel_created = True
                mysql.connection.commit()
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if query_db("select name from Panel_Parameter where panel_no = %s and parameter_no = %s", (panel_id, param_id)) is not None:
                cur.execute("update Panel_Parameter set name = %s, max_marks = %s where panel_no = %s and parameter_no = %s", (param_name, param_marks, panel_id, param_id))
                cur.execute("UPDATE Panel_Marks set parameter%s_marks=0 where panel_no=%s", (int(param_id), panel_id))
                flash("Successfully updated paramater", "success")
            else:
                cur.execute("insert into Panel_Parameter values(%s, %s, %s, %s)", (panel_id, param_id, param_name, param_marks))
                mysql.connection.commit()
        ...
        mysql.connection.rollback()
        flash("Something went wrong!", 'danger')
        return redirect(url_for('admin.def_params'))
    finally:
        cur.close()
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if sha.verify(password, phash[0][0]):
                panel_id = (val//8) + 1
                param_id = (val%8) + 1
                cur.execute("delete from Panel_Parameter where panel_no = %s and parameter_no = %s", (panel_id, param_id))
                # execute_db("UPDATE Panel_Marks set parameter%s_marks=0 where panel_no=%s", (param_id, panel_id))
                mysql.connection.commit()
        ...
        mysql.connection.rollback()
        flash("Something went wrong!", 'danger')
        return redirect(url_for('admin.def_param'))
    finally:
        cur.close()
    ...
    try:
        ...
        if request.method=='GET':
            ...
            if sha.verify(password, phash[0][0]):
                ...
                cur.execute("Update Mentors SET group_limit=%s where mentor_id=%s",(limit, mentorid[0][0], ))
                mysql.connection.commit()
...
def delete_group():
    try:
        ...
        if request.method=="POST":
            ...
            cur.execute("delete from Group_Mentors where group_id = %s", (group_id,))
            cur.execute("delete from Students where group_id = %s", (group_id,))
            cur.execute("delete from Teams where group_id = %s", (group_id,))
            cur.execute("delete from Panel_Group where group_id = %s", (group_id,))
            cur.execute("delete from Panel_Marks where roll_no = %s or roll_no = %s or roll_no = %s", (leader_roll, team[0][3], team[0][4]))
            if team[0][5] is not None:
                cur.execute("delete from Panel_Marks where roll_no = %s", (team[0][5],))
            mysql.connection.commit()
